...
async function resolveSavedUserSession({ allowSoftRecover = true } = {}) {
  ...
  if (session?.user) {
    const user = await fetchUserProfileForAuth(session.user, savedUsername);
    if (!user) {
      return { user: null, session, needsSocialSignup: true };
    }

    if (!user.auth_user_id) {
      await supabaseClient.from('users').update({ auth_user_id: session.user.id }).eq('id', user.id);
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // 연동 상태 UI 관리 (프로필 패널)
  updateSocialLinkingStatus(resolved.session);
...
async function initChat() {
  ...
  if (data && data.length > 0) {
    ...
    [...data].reverse().forEach(item => appendChatMessage({ ...item, users: userMap[item.username] || null }));
  }
    ...
    chatSubscription = supabaseClient.channel('public:chat_messages')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, async (payload) => {
        ...
        const { data: uData } = await supabaseClient
          ...
          .maybeSingle();
        appendChatMessage({ ...rawMsg, users: uData ? { ...uData } : null });
      })
      .subscribe();
  }
