...
(function () {
    ...
    window.initFlappyGame = function () {
        ...
        board.append(player, scoreEl, hint);
        ...
        function loop() {
            ...
            pipes.forEach(pipe => {
                pipe.x -= 2.3 + Math.min(1.5, score * 0.05);
    ...
    window.rewardPoints = async function (amount) {
        ...
        try {
            ...
            if (typeof window.showRewardToast === 'function') {
                window.showRewardToast(`포인트 +${amount.toLocaleString()}P를 받았습니다.`);
            }
